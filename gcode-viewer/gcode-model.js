function createObjectFromGCode(gcode) {
    ...
    function addSegment(p1, p2) {
        ...
        p_coords.push("p"+padLeadingZeros(index+1,4)+"="+p2.x.toFixed(3)+","+p2.y.toFixed(3)+","+p2.z.toFixed(3)+","+zero.toFixed(3)+","+zero.toFixed(3)+","+zero.toFixed(3));
        
        if(typeof p2.f !== undefined)
            {
           // let speed=p2.f/100;
           let speed=10;
            mvm.push("MOVL P"+padLeadingZeros(index+1,4)+" V="+speed.toFixed(2) + " PL=5");
            } 
        else mvm.push("MOVL P"+padLeadingZeros(index+1,4));
        
        if(typeof p2.extruding !== undefined)
            {
                if (welding !== p1.extruding)
                {
                    welding = p1.extruding;
                    if(welding) mvm.push("ARCON");
                    else mvm.push("ARCOF");
                }
                
                if (welding !== p2.extruding)
                {
                    welding = p2.extruding;
                    if(welding) mvm.push("ARCON");
                    else mvm.push("ARCOF");
                }
            ...
            //hegesztési előtolás még nincs beállítva
        index++;
    ...
    localStorage.setItem("p_coords",JSON.stringify(p_coords));
